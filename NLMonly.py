...
class SimplifiedCTM(nn.Module):
    ...
    now including Neuron-Level Models (NLMs) and a history buffer (state_trace).
    """
    ...
    def __init__(
        ...
        d_backbone=512,
        iterations=5,
        memory_length=10, # New: History length for NLMs
        memory_hidden_dims=64, # New: Hidden dims for NLM MLP
        deep_nlms=True, # New: Use deep NLMs (MLP) or single layer
        do_layernorm_nlm=False, # New: Apply LayerNorm in NLM (typically False in original)
        dropout=0.1,
        num_heads=4,
        num_classes=10,
        ...
        self.memory_length = memory_length # Store M
        ...
        self.kv_proj = nn.Linear(d_backbone, d_attn)
        self.attention = nn.MultiheadAttention(
            embed_dim=d_attn, num_heads=num_heads, batch_first=True
        )

        # "GAP" / Processing Block (Synapse Model) - processes concatenated input
        self.synapse = nn.Sequential(
            nn.Dropout(dropout),
            nn.Linear(d_attn + d_model, d_model * 2), # Output size is d_model*2 for GLU
            nn.GLU(dim=-1), # GLU operates on the last dimension, outputs d_model
            nn.LayerNorm(d_model),
        )

        # --- CHANGED: Neuron-Level Models (NLMs) using get_neuron_level_models logic ---
        ...
        # Initial state for the learnable FIFO buffer (state_trace)
        # Shape: (d_model, memory_length)
        self.initial_state_trace = nn.Parameter(torch.zeros(d_model, memory_length))

        # Initial state for z_t (post-activation)
    ...
    def forward(self, x, track=False):
        ...
        kv = self.kv_proj(pooled_features).unsqueeze(1)  # Shape: (B, 1, D_attn)

        # --- Initialise Recurrent State ---
        # z_t: current post-activation state
        z_t = self.start_state.unsqueeze(0).expand(B, -1)  # Shape: (B, D_model)

        # state_trace: FIFO buffer for pre-activations history
        # Shape: (B, D_model, M)
        state_trace = self.initial_state_trace.unsqueeze(0).expand(B, -1, -1).clone() # Clone to avoid in-place modification of parameter
        ...
        for t in range(self.iterations):
            # --- Cross Attention ---
            q = self.q_proj(z_t).unsqueeze(1)  # Shape: (B, 1, D_attn)
            attn_out, _ = self.attention(
                q, kv, kv, need_weights=False
            )  # Shape: (B, 1, D_attn)
            attn_out = attn_out.squeeze(1)  # Shape: (B, D_attn)

            # --- Concatenate with z_t ("Could be concat") ---
            concat_input = torch.cat(
                [attn_out, z_t], dim=1
            )  # Shape: (B, D_attn + D_model)

            # --- Apply Synapse ("GAP" / Processing) ---
            # This produces the new 'pre-activation' (state) for this tick
            state = self.synapse(concat_input)  # Shape: (B, D_model)

            # --- Update state_trace FIFO buffer ---
            # Shift history left (remove oldest) and add new state (state) at the end
            state_trace = torch.cat([state_trace[:, :, 1:], state.unsqueeze(-1)], dim=-1) # Shape: (B, D_model, M)

            # --- Update z_t to z_{t+1} (via "NLM" processing state_trace) ---
            # The trace_processor (NLM) expects input of shape (B, D_model, M)
            # It outputs (B, D_model) - the new post-activation state
            z_t = self.trace_processor(state_trace) # Shape: (B, D_model)
...
def get_dataloader(dataset_name, data_dir, batch_size=64, train=True, val_ratio=0.2):
    if dataset_name.lower() == "cifar10":
        ...
        # 2. Define NO Augmentations for TESTING (train=False)
            ...
            transform = transforms.Compose(
                [
                    transforms.Resize(224),       # Resize to a larger size
                    transforms.ToTensor(),
                    transforms.Normalize(
                        mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]
                    ),
